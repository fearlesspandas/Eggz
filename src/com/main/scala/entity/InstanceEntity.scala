...
trait InstanceEntity
  ...
  val starting_health: Double

  val starting_energy: Double
  ...
  def setHealth(
    ...
    for {
      _ <- healthRef.update(_.updated(player_id, health))
    } yield this
  ...
  def setEnergy(
    ...
    for {
      _ <- healthRef.update(_.updated(player_id, value))
    } yield this

  def health(player_id: GLOBZ_ID): IO[HealthError, Double] =
    healthRef.get.map(_.getOrElse(player_id, starting_health))

  def energy(player_id: GLOBZ_ID): IO[HealthError, Double] =
    energyRef.get.map(_.getOrElse(player_id, starting_energy))
