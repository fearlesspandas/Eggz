...
object GlobzApp extends zio.App {
  ...
  override def run(args: List[String]): URIO[zio.ZEnv, ExitCode] = program().provideLayer(localApplicationEnvironment) *> ZIO.succeed(ExitCode.success)

  def program(): ZIO[ApplicationEnvironment, Nothing, ExitCode] = (
    (for {
      _ <- putStrLn("Welcome")
      ref <- Ref.make[Int](0)
      _ <- (getStrLn.orDie.repeatWhileM {
        ...
        case st =>
          (for {
            ...
            _ <- ref.update(_ + 1)
            all <- Globz.getAll()
            _ <- Globz.tickAll().zipPar(putStrLn(s"all eggs current: ${all}").fold(e => (), x => x))
      ...
      //      _ <- putStrLn(s"all eggs current: ${all}").fold(e => (),x=>x)
      //      _ <- program()
    } yield ExitCode.success).fold(e => {
      println(e.toString); null.asInstanceOf[Nothing]
    }, x => x) //.tapError( e => putStrLn(s"Unexpected failure $e")).fold(_ => null.asInstanceOf[Nothing],x => x)
    //      .flatMapError(e =>
