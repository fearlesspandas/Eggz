...
import src.com.main.scala.entity.Storage.{GenericServiceError, REF_STORE}
...
case class basicStorage[I](refs: REF_STORE[I] = Set()) extends Storage.Service[I] {
  ...
  override def add(item: I*): IO[Storage.ServiceError, Storage.Service[I]] = IO {
...
case class BasicStorage(sizeLimit: Int, weightLimit: Double, contents: Set[Any] = Set()) extends storage {
  def setContents(value: Set[Any]): storage = this.copy(contents = value)

  def addContents(value: Set[Any]): storage = this.copy(contents = this.contents ++ value) //definitely fix this to be true addition (currently overwrites dups)
  def removeContents() = ???
}