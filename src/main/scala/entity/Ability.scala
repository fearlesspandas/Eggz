...
case class Slizzard(from: GLOBZ_ID, location: FieldOps.Location)
    extends Ability {
    ...
    for {
      res <- ZIO
        .succeed(
          MultiResponse(
            Chunk(
              QueuedServerMessage(Chunk(DoAbility(from, 2, location))),
              QueuedClientBroadcast(Chunk(DoAbility(from, 2, location)))
            )
          )
        )
    } yield res
