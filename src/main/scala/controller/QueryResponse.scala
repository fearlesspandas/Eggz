...
case class ActiveDestination(id: GLOBZ_ID, destination: UUID)
    extends QueryResponse
...
case class NextIndex(id: GLOBZ_ID, index: Int) extends QueryResponse
object NextIndex {
...
case class TeleportToNext(id: GLOBZ_ID, location: (Double, Double, Double))
    extends QueryResponse
...
case class AllDestinations(id: GLOBZ_ID, destinations: Seq[DEST])
    extends QueryResponse
...
case class NewDestination(id: GLOBZ_ID, destination: DEST) extends QueryResponse
...
case class DeleteDestination(id: GLOBZ_ID, uuid: UUID) extends QueryResponse
...
case class ClearDestinations(entity_id:String) extends QueryResponse
...
object Unfollowing {
  ...
  implicit val decoder: JsonDecoder[Unfollowing] =
    DeriveJsonDecoder.gen[Unfollowing]
}
...
case class QueuedServerMessage(responses: Chunk[QueryResponse])
    extends QueryResponse
case class QueuedClientMessage(id: GLOBZ_ID, responses: Chunk[QueryResponse])
    extends QueryResponse
...
object AbilityPocketed {
  implicit val decoder: JsonDecoder[AbilityPocketed] =
    DeriveJsonDecoder.gen[AbilityPocketed]
...
case class Inventory(id: GLOBZ_ID, items: Map[Item, Int]) extends QueryResponse
object Inventory {
