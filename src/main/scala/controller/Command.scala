...
trait Command[-Env, +Out] extends Product with Serializable { self =>
  ...
  def flatMap[R1 <: Env, B](f: Out => Command[R1, B]): Command[R1, B] =
    CMD(self, f)
  def map[R1 <: Env, B](f: Out => B): Command[R1, B] =
    flatMap(a => Command.succeed(f(a)))
}
case class RUN_COMMAND_ASYNC[-Env](
  ...
  override def run: ZIO[Env, CommandError, Unit] =
    cmd.run
      .repeat(Schedule.spaced(Duration.ofMillis(duration)))
      .fork
      .map(_ => ())
}
object Command {
  case class CMD[Env, A, B](
    first: Command[Env, A],
    success: A => Command[Env, B]
